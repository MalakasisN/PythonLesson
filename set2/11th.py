def f(x):
    ...
    for i in range(len(x)):
        c+=x[i]
        l.append(c)
    ll=[(l[i]-l[j],j+1,i+1) for i in range(len(l)) for j in range(len(l)) if i>j]
    print((max(ll)[1],max(ll)[2]))#dedomenou oti ksekinaei apo 0 lefta,alliws ksekinontas apo to 0 dinei (27,55)
